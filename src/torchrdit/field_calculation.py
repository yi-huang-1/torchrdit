...
class FieldCalculationMixin:
    ...
    def _calculate_polarization(self, sources=None, kinc=None):
        ...
        # Select normal vs oblique via torch.where (compile-friendly).
        # theta_batch: (B,) for scalar-per-source, or (B, F) for per-frequency.
        # Expand to (B, F) then unsqueeze to (B, F, 1) for correct per-frequency masking.
        is_normal = torch.abs(theta_batch) < 1e-3
        if is_normal.dim() == 1:
            is_normal = is_normal.unsqueeze(-1).expand(-1, self.n_freqs)  # (B,) -> (B, F)
        normal_mask = is_normal.unsqueeze(-1)  # (B, F, 1)
