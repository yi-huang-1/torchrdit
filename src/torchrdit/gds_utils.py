...
def generate_ring_mask_vectorized(
    shape: Tuple[int, int], center: Tuple[float, float], inner_radius: float, outer_radius: float
) -> np.ndarray:
...
def create_circular_hole_vectorized(
    mask: np.ndarray, hole_center: Tuple[float, float], hole_radius: float, bounds: Tuple[int, int, int, int] = None
) -> np.ndarray:
...
def point_in_triangle_vectorized(
    shape: Tuple[int, int], v0: Tuple[float, float], v1: Tuple[float, float], v2: Tuple[float, float]
) -> np.ndarray:
...
def create_multiple_holes_vectorized(mask: np.ndarray, holes: List[dict]) -> np.ndarray:
