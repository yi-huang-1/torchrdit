...
def mask_to_gds(
    ...
    for pattern_region in pattern_regions:
        ...
        if padding > 0:
            ...
            pattern_bond = [
                (
                    min(max(int(round(p[0])), 0), max_row),  # row
                    min(max(int(round(p[1])), 0), max_col),
                )  # col
                for p in outer_contour
            ]
        ...
        for hole_region in hole_regions:
            ...
            if (
                hole_bbox[0] >= pattern_bbox[0]
                and hole_bbox[1] >= pattern_bbox[1]
                and hole_bbox[2] <= pattern_bbox[2]
                and hole_bbox[3] <= pattern_bbox[3]
            ):
        ...
        if isinstance(pattern_group, list) and len(pattern_group) > 0:
            ...
            for boundary_idx, boundary in enumerate(pattern_group):
                if len(boundary) > 2:  # Need at least 3 vertices for a polygon
                    poly_mask = shape_generator.generate_polygon_mask(polygon_points=boundary, soft_edge=soft_edge)
