...
class TestNaNPreventionCoverage:
    ...
    def test_nan_prevention_rdit_sequential_and_batched(self, solver_multi_wavelength):
        """R-DIT: outputs remain finite AND energy-conserving for sequential and batched sources."""
        ...
        # Sequential — check finiteness AND energy conservation
        for src in sources:
            res = solver.solve(src)
            assert torch.isfinite(res.reflection).all()
            assert torch.isfinite(res.transmission).all()
            R_total = torch.sum(res.reflection).item()
            T_total = torch.sum(res.transmission).item()
            assert R_total >= -1e-3 and T_total >= -1e-3, f"Negative R={R_total} or T={T_total}"

        # Batched — check finiteness AND energy conservation
        ...
        for i in range(len(sources)):
            ...
            R_total = torch.sum(batched[i].reflection).item()
            T_total = torch.sum(batched[i].transmission).item()
            assert R_total >= -1e-3 and T_total >= -1e-3, f"Negative R={R_total} or T={T_total}"
    ...
    def test_nan_prevention_rcwa_sequential_and_batched(self):
        """RCWA: outputs remain finite AND energy-conserving for sequential and batched sources."""
        ...
        for i in range(len(sources)):
            ...
            R_total = torch.sum(results[i].reflection).item()
            T_total = torch.sum(results[i].transmission).item()
            assert R_total >= -1e-3 and T_total >= -1e-3, f"Negative R={R_total} or T={T_total}"
